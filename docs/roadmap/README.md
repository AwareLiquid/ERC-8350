...
| Independent second implementation passing the golden vectors | **Still open.** Three independent external recomputations exist (all by @babyblueviper1 / WYRIWE), and it is worth being exact about what each does and does not cover — none of them is a second implementation of the core registry:<br>1. **EIP-712 constants** (2026-07-26): the three typehashes and the domain separator re-derived from the type strings and domain fields, byte-for-byte.<br>2. **Interop layer** — [`preaction-governance-conformance`](https://github.com/babyblueviper1/preaction-governance-conformance): an executable checker for `attestation-refs.md` §2/§3/§6, run against this repository's live Sepolia fixture rather than synthetic data.<br>3. **Core layer, one delta** — [`invinoveritas/examples/erc8274-erc8350-composition`](https://github.com/babyblueviper1/invinoveritas/tree/main/examples/erc8274-erc8350-composition): `transitionId`, `provenanceCommitment`, `deltaCommitment` and `nextStateRoot` derived from the published formulas alone, re-verified here against `@erc-awar/core` from independently reconstructed bytes — every value matching.<br><br>What remains unclaimed is the part that matters most: **continuity**. It is worth being blunt about why. Until 2026-08-01 this row asked implementers to "drive `test-vectors/v1.json` end to end, including the rejection cases" — but v1 contains a single genesis delta and no rejection cases at all, so sequence monotonicity, prior-root linkage and every refusal were untestable from the published artifacts. The ask was unmeetable, and the gap was ours, not the ecosystem's. [`test-vectors/v2.json`](../../test-vectors/v2.json) fixes it: a 5-transition chain where each delta is only computable from the previous one's `nextStateRoot`, plus 12 rejection cases with their errors and the order they are checked in. `pnpm check:vectors` runs it against the TypeScript reference and `contracts/test/unit/VectorsV2.t.sol` drives the same file against the Solidity registry — so the bar is now a file an implementer can make pass, not a paragraph they have to interpret. [`docs/implementing.md`](../implementing.md) is the entry point for someone who has not read this repository: what conformance does and does not mean, the surface to implement, and what to do with a mismatch. |
| External Solidity audit | **Provisional review received; corrections/evidence pending** — the 2026-07-31 email reports no Critical/High/Medium findings, but has scope/API discrepancies and lacks reviewer-owned publication evidence; disposition is recorded in `audits/g4/external/REVIEW_INTAKE_2026-07-31.md`; `pnpm security:rc` remains blocked |
